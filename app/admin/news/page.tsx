...
export default function AdminNewsPage() {
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      const result = await handleGetAllNews(params);
      if (result.success) {
        setNews(result.data);
        setTotal(result.pagination.total);
        setTotalPages(result.pagination.totalPages);
      } else {
        showToast(result.message || "Failed to load news", "error");
      }
    } catch {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    const result = await handleDeleteNews(deleteTarget.id);
    if (result.success) {
  ...
  const handleToggle = async (id: string) => {
    ...
    if (result.success) { showToast(result.message); fetchNews(); }
    else showToast(result.message || "Failed", "error");
  };

  const handlePublish = async (id: string) => {
    const result = await handlePublishNews(id);
    if (result.success) { showToast("Article published"); fetchNews(); }
    else showToast(result.message || "Failed", "error");
  };

  const handleArchive = async (id: string) => {
    const result = await handleArchiveNews(id);
    if (result.success) { showToast("Article archived"); fetchNews(); }
    else showToast(result.message || "Failed", "error");
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-zinc-100">Delete Article</h3>
      ...
      <div className="mb-8">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">News Management</h1>
            <p className="text-zinc-500 text-sm mt-1">Manage all news articles across categories</p>
          </div>
