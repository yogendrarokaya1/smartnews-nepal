...
const CATEGORIES = [
  "all", "politics", "sports", "technology",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-sm text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-zinc-600"
          />
        </div>
      ...
      <div className="text-xs text-zinc-500">
        {loading ? "Loading..." : `${total} video${total !== 1 ? "s" : ""} found`}
      </div>
                  ...
                  <tr key={item._id} className="bg-zinc-950 hover:bg-zinc-900/40 transition-colors group">
                    <td className="px-4 py-3 max-w-xs">
                      <div className="flex flex-col">
                        ...
                        {item.author && (
                          <span className="text-xs text-zinc-600 mt-0.5">{item.author.fullName}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/admin/videos/${item._id}`}
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-800 bg-zinc-900/40">
            ...
            <div className="flex gap-1">
              <button onClick={() => onPageChange(page - 1)} disabled={page === 1}
                className="px-3 py-1.5 rounded-md text-xs text-zinc-400 border border-zinc-800 hover:border-zinc-600 disabled:opacity-30 disabled:cursor-not-allowed">
                ← Prev
              </button>
