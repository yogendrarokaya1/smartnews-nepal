...
import { handleCreateNews, handleUpdateNews } from "@/lib/actions/news.action";
...
export default function NewsForm({ initialData = {}, mode }: NewsFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Build FormData — same pattern as updateProfile in auth-action.ts
      ...
      const tagsArray = form.tags
        ...
        .map(t => t.trim())
        .filter(Boolean);
      // Append each tag individually so multer/express receives tags[] array
      tagsArray.forEach(tag => fd.append("tags[]", tag));

      if (thumbnail) fd.append("thumbnail", thumbnail);

      // Call server action — same pattern as handleUpdateProfile
      const result = mode === "create"
        ? await handleCreateNews(fd)
        : await handleUpdateNews(initialData._id!, fd);
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              required maxLength={200} placeholder="Enter article title..."
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              required maxLength={500} rows={3} placeholder="Brief description..."
              className="w-full px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-zinc-600 transition-colors resize-none"
            />
            <div className="text-right text-xs text-zinc-600">{form.summary.length}/500</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              required rows={16} placeholder="Full article content. HTML is supported..."
              className="w-full px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-zinc-600 transition-colors resize-y font-mono text-sm"
            />
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-100 focus:outline-none focus:border-zinc-600 transition-colors cursor-pointer capitalize"
            >
              {CATEGORIES.map(cat => (
