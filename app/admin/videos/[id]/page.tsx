...
export default async function ViewVideoPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-xs text-zinc-500 mb-3">
          ...
          <span>/</span>
          <span className="text-zinc-300 truncate max-w-xs">{video.title}</span>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link href={`/admin/videos/${id}/edit`}
      ...
      <div className="max-w-4xl space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className={`px-2 py-1 rounded-md text-xs font-semibold ${STATUS_STYLES[video.status]}`}>
            {video.status}
          </span>
          <span className={`px-2 py-1 rounded-md text-xs font-semibold uppercase tracking-wide ${CATEGORY_STYLES[video.category] || "bg-zinc-800 text-zinc-400"}`}>
            {video.category}
          </span>
          {video.isFeatured && (
          ...
          <span className="text-xs text-zinc-500 ml-auto">
            {video.views?.toLocaleString()} views • {formatDuration(video.duration)}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 pb-4 border-b border-zinc-800">
          ...
          <div>
            <p className="text-sm font-medium text-zinc-200">{video.author?.fullName || "Admin"}</p>
        ...
        <div className="aspect-video rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800">
          {video.videoUrl.includes("youtube.com") || video.videoUrl.includes("youtu.be") ? (
            <iframe
              src={`https://www.youtube.com/embed/${new URL(video.videoUrl).searchParams.get("v") || video.videoUrl.split("/").pop()}`}
        ...
        {video.thumbnail && (
          <div className="space-y-2">
            ...
            <img
              src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${video.thumbnail}`}
              alt={video.title}
              className="w-full max-w-sm rounded-lg border border-zinc-800"
            />
          </div>
        )}
        ...
        <div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg">
          ...
          <p className="text-sm text-zinc-400 font-mono">{video.slug}</p>
        </div>

        {/* Video URL */}
        <div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg">
          <p className="text-xs text-zinc-500 mb-1">Video URL</p>
          <a
            href={video.videoUrl}
            target="_blank"
